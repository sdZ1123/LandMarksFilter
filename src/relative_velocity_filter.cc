...
namespace mediapipe {
	...
	float RelativeVelocityFilter::Apply(const double timestamp, float value_scale,
		float value) {
		const int64_t new_timestamp = timestamp;
		...
		if (last_timestamp_ == -1) {
			...
			if (distance_mode_ == kLegacyTransition ||
				distance_mode_ == kForceCurrentScale)
				std::cout << "DCHECK Successfully!" << std::endl;
			else
				std::cerr << "DCHECK Failed!" << std::endl;
			
			const float distance =
				distance_mode_ == kLegacyTransition
				? value * value_scale -
